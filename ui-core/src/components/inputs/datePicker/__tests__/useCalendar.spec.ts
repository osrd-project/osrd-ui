...
describe('useCalendar', () => {
  ...
  describe('day calculation', () => {
    it('should include days from the previous month when the first day of the displayed month is not a Monday', () => {
      ...
      const { result } = renderHook(() =>
        useCalendar({ displayedMonthStartDate, onClickDay: () => {} })
      );
  ...
  describe('day wrapper classnames', () => {
    it('should have "day-wrapper" classname for all days', () => {
      ...
      const { result } = renderHook(() =>
        useCalendar({ displayedMonthStartDate, selectedSlot, selectableSlot, onClickDay: () => {} })
      );
  ...
  describe('isDateSelectable', () => {
    ...
    it('should return false for days before today', () => {
      expect(result.current.isDateSelectable(new Date(1900, june, 30))).toBe(false);
    });

    it('should return true for days within the selectable slot, in the displayed month, and after today', () => {
